...
class FrontendBaseController extends FrontendController
{
	...
	public function customer_verify()
	{
		...
		if($_ctr!=0)
		{
			...
			foreach($_data as $user)
			{
				...
				{
					$_users["_mail"]=$username;
					 View::share(array('page'=> 'Login',
					'account' =>'null'        
        				));
        			$this->layout->content = View::make('frontend.customer_login2',$_users);
				}
			}
		}
		else
	...
	public function forgot_password($mail)
	{
		...
		$_data=ModelUser::where('user_mail', $mail)
				->where('user_type', 4)
				->get();
			...
			foreach($_data as $user)
			{
				$data=array(
        			...
        			$this->layout->content = View::make('frontend.forgot_panel',$data);
        	}
	}
	...
	{
		session_start();
		$_userid=ModelUser::where('user_mail', $_SESSION["mail"])->get();
    		foreach($_userid as $userid) {
			$user_id=array(
				'_user_id'=>$userid->id,
				'_user_fname'=>$userid->user_fname,
		...
		$data['user_mname'] = $user_id['_user_mname'];
		$data['user_email'] = $user_id['_user_mail'];
		$data['recovery_code'] = strtoupper(Str::random(15));
		
		
			Mail::send('frontend.msg_reco', $data, function($msg)use($data){
			$msg->to($data['user_email'],ucfirst($data['user_lname']).','.ucfirst($data['user_fname']))->subject('Recover Account');		
			...
			ModelUser::where('user_mail', $_SESSION["mail"])->update($_user);
			
			
        	return Redirect::action('FrontendBaseController@recov_panel');
	}
	
	public function recov_panel()
	{
		session_start();
		View::share(array(
			'page'=>'Home',
        ...
        $this->layout->content = View::make('frontend.resetPanel');
	}
	
	
	public function recoVerify()
	{
		session_start();
		$verify=Input::get("vcode");
		
		$ctr = ModelUser::where('user_mail', $_SESSION["mail"])
				->where('recovery_code', $verify)
				->count();
        ...
        $this->layout->content = View::make('frontend.resetPassword');
		}
		else
		{
        	return Redirect::action('FrontendBaseController@recov_panel');
	...
	public function user_update()
	{
		...
		if($_data["alph"]==0||$_data["nume"]==0)
		{
        	...
        	$this->layout->content = View::make('frontend.resetPassword',$_data);
		}
		else
		{
			$data["user_pass"]=Hash::make($_data["password"]);
			ModelUser::where('user_mail', $_SESSION["mail"])->update($data);
			
        	return Redirect::action('FrontendBaseController@loginview');
		}
	}
	
	public function user_updateaddr()
	{
		session_start();
		try{
			...
			$data["addr_imgsrc"]=Input::file("fileToUpload")->getClientOriginalName();
			ModelUser::where('user_mail', $_SESSION["username"])->update($data);
			$fileUploadOrigName=$fileUpload->getClientOriginalName();
			Input::file("fileToUpload")->move($path, $fileUploadOrigName);
			$data["error"]="<script>alert('Upload file successful!')</script>";
			View::share(array(
					'page'=>'Home',
					'account'=>'!null'
					));
			...
			$errormsg["error"]="<script>alert('Cannot upload data! Maybe it reached the maximum limit for upload which is 2mb.Please try to upload different data.Thank You')</script>";
			 session_start();
		View::share(array(
			'page'=>'Home',
			'account'=>'!null'
		));
        $this->layout->content = View::make('frontend.done', $errormsg);
		}
