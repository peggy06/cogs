...
class BackendBaseController extends BackendController
{
	...
	public function verify()
	{
		...
		$_ctr=ModelAdmin::where('user_mail', $username)->count();
    ...
    {
    	session_start();
    	$findUser=ModelOrder::where('order_id', $id)->get();
		...
		$data["order"]=ModelOrderItem::where('order_id', $id)->get();
		$data["user"]=ModelUser::where('id', $_data['Userid'])->get();
		$data["total"]="";
		View::share(array(
        	'page'=>'Order Details',
        	'modify'=>'otherpage'
       ));
        $this->layout->content = View::make('backend.order_details',$data);
	}
	
	public function order_dispose($id)
	{
		session_start();
		$data["complete"]=0;
		ModelOrder::where('order_id', $id)->update($data);
		$tZone = 'Asia/Singapore';
	    			$dt = Carbon::now($tZone)->toDateString();
	    			$tm = Carbon::now($tZone)->toTimeString();
					...
					$_userlog->Date= $dt;
					$_userlog->user_type= 2;
					$_userlog->activity="Disposed an order. ID:".$id;
					$_userlog->save();
        return Redirect::action('BackendBaseController@order_list');
	}
	
	public function confirm_dispose($id)
	{
		session_start();
		$data["details"]=ModelOrder::where('order_id', $id)->get();
		View::share(array(
        	'page'=>'Order Disposal',
        	'modify'=>'otherpage'
       ));
        $this->layout->content = View::make('backend.confirm_dispose',$data);
	}
}